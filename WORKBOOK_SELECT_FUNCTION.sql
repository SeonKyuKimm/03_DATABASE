...
FROM TB_STUDENT
WHERE EXTRACT(YEAR FROM ENTRANCE_DATE) -- 입학년도 (2002년)
-EXTRACT(YEAR FROM TO_DATE('19' || SUBSTR(STUDENT_SSN, 1, 6), 'YYYYMMDD')) -- 학생 출생 년도
> 19; 
-- 입학일에서 출생년도 빼면 입학한 나이가 나옴. 얘가19보다 크면 재수생!!!
...
-- 6 : 금요일
-- 7 : 토요일
SELECT TO_CHAR( TO_DATE('2020-12-25'), 'DAY' ) "2020-12-25 크리스마스는"
FROM DUAL;
...
FROM DUAL;
       ...
       --TO_CHAR(SYSDATE, 'MM / DD (DY)') FROM DUAL;
...
SELECT DEPARTMENT_NO 학과번호, COUNT(*) "학생 수"
FROM TB_STUDENT
