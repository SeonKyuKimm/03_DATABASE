...
-- ex>인스타 3초전 , 16분전 --시간 지나면--> 2024-04-11
-- 댓글 개수 , 좋아요 개수 옆에 띄워줄거임
-- 번호 / 제목 [댓글 개수] / 작성자 닉네임 / 작성일 / 조회수 / 좋아요 개수
-- 상관 서브 쿼리
-- 메인쿼리를 1행 조회하고
-- 1행 조회 결과를 이용해서 서브쿼리 수행(메인 쿼리 모두 조회할 때 까지 반복)
	...
	(SELECT COUNT(*) 
	 ...
	 (SELECT COUNT(*)
		...
		CASE
			...
			THEN FLOOR( (SYSDATE - BOARD_WRITE_DATE) * 24 * 60 * 60) || '초 전' -- 하루를 초 단위로 환산
