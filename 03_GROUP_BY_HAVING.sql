 ...
 * 6 : ORDER BY 컬럼명 | 별칭 | 컬럼순번 정렬방식(ACS / DESC) [NULLS FIRST | LAST];
 * 
 * */
...
-- 그룹함수는 단 한개의 결과값만 산출하기 때문에 그룹이 여러개일 경우 오류가 발생할 수 있다
-- 여러개의 결과값을 산출하기 위해서 그룹함수가 적용된 그룹의 기준을 ORDER BY 절에 기술하여 사용할 때도 있다
...
FROM EMPLOYEE
GROUP BY DEPT_CODE; --> DEPT_CODE 가 같은 행 끼리 하나의 그룹이 됨
...
SELECT DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별, 
			 ...
			 SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용X(해석 순서 상, SELECT절 해석이 안되서)
ORDER BY "인원 수"; -- 별칭 사용 O (SELECT 절 해석이 완료된 이후이기때문에 가능)
...
SELECT DEPT_CODE, ROUND( AVG(SALARY) ), COUNT(*) "인원 수"
FROM EMPLOYEE
...
-- 부서 별 평균 급여가 3백만원 이상인 부서를 조회(부서코드 오름차순)

SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
--WHERE SALARY >= 3000000 --> 한 사람의 급여가 300만원 이상이라는 조건( 요구사항과 다름 )
GROUP BY DEPT_CODE
...
-- ROLL UP : GROUP BY 절에서 가장 먼저 작성된 컬럼의 중간 집계를 처리하는 함수이다
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
...
- UNION( 합집합) : 두 SELECT 결과를 하나로 합친 것
										단, 중복은 한 번만 작성하게 된다.
										
-	INTERSECT ( 교집합) : 두 SELECT 결과 중, 중복되는 부분만 조회함									

- UNION ALL : UNION + INTERSECT 의 값
							합집합에서 중복부분을 제거하지 않은것
...
-- EMPLOYEE 테이블에서 부서코드가 'D5'인 사원의 사번, 이름 , 부서코드 ,급여
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY  
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'

UNION --(심봉선 대북혼이 한번만 조회됨.)
--INTERSECT(심봉선 대북혼이 조회됨. 두 식의 교집합이라서) 
--UNION ALL (심봉선 대북혼이 두번씩 작성되어 있음)
--MINUS (위 아래의 교집합- 심봉선 대북혼-을 제거하고 남은것만 조회)
