/*
	DML (Data Manipulation Langeuage) : 데이터 조작 언어

테이블에 있는 값을 삽입하거나(INSERT) 수정하거나(UPDATE) 삭제(DELETE) 하는 구문이다
...
CREATE TABLE SAL_GRADE2 AS SELECT * FROM SAL_GRADE;
...
-- 단 컬럼의 순서를 지켜서 VALUES에 값을 지켜서 값을 기입해야 한다
...
-- 선택 안된 컬럼은 값이 NULL로 알아서 들어감 (DEFAULT 존재 시 , DEFAULT 값으로 삽입됨)

INSERT INTO EMPLOYEE2 ( EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
VALUES('900', '장채현', '901128-1000000', 'jang_ch@kh.or.kr', '01012341234', 'D1', 'J7', 'S3', 4300000);
...
-- 서브쿼리(SELECT) 결과를 EMP_01 테이블에 INSERT한것임
--> SELECT 조회 결과의 데이터 타입, 컬럼 개수가
-- INSERT 하려는 테이블의 컬럼과 일치해야 사용 가능하다
INSERT INTO EMP_01(
	SELECT EMP_ID, EMP_NAME, DEPT_TITLE
	FROM EMPLOYEE2
	LEFT JOIN DEPARTMENT2 ON(DEPT_CODE = DEPT_ID)
...
-- DEPARTMENT2 테이블에서 DEPT_ID 가 'D9인' 부서 조회
SELECT * FROM DEPARTMENT2;

-- DEPARTMENT2 테이블에서 DEPT_ID가 'D9'인 행의 DEPT_TITLE은
-- '전략기획팀' 으로 수정
...
-- * 여러 컬럼을 한번에 수정할 시 콤마(,)로 컬럼을 구분하면 됩니다.
-- D9 / 총무부 -> D0 / 전략기획팀으로 수정

UPDATE DEPARTMENT2 
SET DEPT_ID ='D0',
		DEPT_TITLE = '전략기획팀' -- WHERE절 직전 컬럼에는 콤마 ㄴㄴ
WHERE DEPT_ID = 'D9'
AND DEPT_TITLE ='총무부';
...
-- 급여봐 보너스율을 유재식 사원과 동일하게 변경해 주기로 했다.
-- 이를 반영하는 UPDATE문을 작성해라

-- 유재식의 급여
SELECT SALARY 
FROM EMPLOYEE2
WHERE EMP_NAME = '유재식'; -- 340만
...
SELECT * FROM EMP_M02; -- 5명 (기존4 + 곽두원)

UPDATE EMP_M02 SET SALARY = '0';

SELECT SALARY FROM EMP_M02;
...
-- 만약에 WHERE 조건을 설정하지 않으면 모든 행이 다 삭제된다.
...
-- EMPLOYEE2 에서 이름이 '장채현'인 사원 정보 삭제
...
SELECT * FROM EMPLOYEE2
WHERE EMP_NAME = '장채현'; --> 조회결과 있음 


-- ROLLBACK 구문은 마지막 커밋 시점까지 돌아간다.
...
-- 5 . TRUNCATE (DDL 입니다.. DML 아니예요..)
-- 테이블의 전체 행을 삭제하는 DDL
-- DELETE 보다 수행속도가 좀 더 빠르다.
-- ROLLBACK을 통해 데이터 복구할 수 없다.
...
-- 롤백 후 복구 확인
SELECT * FROM EMPLOYEE3; -- 복구 안됨을 확인했다.!


-- DELETE : 휴지통에 버리기
-- TRUNCATE : 완전삭제 (SHIFT + DELETE 키)
