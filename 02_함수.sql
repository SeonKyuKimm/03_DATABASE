-- 함수 : 컬럼의 값을 읽고, 연산한 결과를 반환

-- 단일행 함수 : N 개의 값을 읽어 N 개의 결과를 반환

-- 그룹 함수 : N 개의 값을 읽어 1개의 결과를 반환 ( 합계 평균 최대 최소)

-- 함수는 SELECT 문의
-- ( SELECT절, WHERE 절 , ORDER BY 절, GROUP BY 절, HAVING 절 )에서 사용가능

--------------------------------------단일 행 함수 ------------------------------

-- LENGTH( 컬럼명 | 문자열 ) 전달 : 길이 반환
...
SELECT INSTR ('AABAACAABBAACASABA' , 'B') FROM DUAL; -- RESULT = 3

-- 문자열을 5번째 문자부터 검색하여 첫번째 B위치를 조회
SELECT INSTR ('AABAACAABBAACASABA' , 'B', 5) FROM DUAL; -- RESULT = 9

-- 문자열을 5번째 문자부터 검색하여 두 번째 B의 위치를 조회
SELECT INSTR ('AABAACAABBAA', 'B', 5, 2) FROM DUAL; -- 10


-- EMPLOYEE TABLE에서 사원명 이메일 이메일 중 '@' 위치조회
...
-- 컬럼이나 문자열에서 지정한 위치부터 지정된 길이만큼 문자열을 잘라내어 반환함
--> 잘라낼 길이 생략 시 끝까지 잘라냄


-- EMPLOYEE 테이블에서 사원명 이메일 중 아이디부분만 조회
...
-- EMPLOYEE TABLE 에서 사원의 월급을 100만으로 나눴을 때 나머지 조회
...
-- EMPLOYEE 데이터에서 사번이 짝수인 사원의 사번 이름 조회
...
-- TRUNC(숫자 | 컬럼명 [, 위치]) : 버림. 특정 위치 아래를 버리는것 (절삭)

SELECT TRUNC(123.456) FROM DUAL; --123, 위치 지정 X, 소수점 아래 버림
...
-- 2024-02-29 11:20:33.770 +0900 (UTC +9 시간 나타냄, 한국 표준시)

--MONTHS_BETWEEN(날짜, 날짜) : 전달받은 두 날짜의 개월 수 차이를 반환
SELECT ROUND( MONTHS_BETWEEN(SYSDATE, '2024-06-26'),3 ) "수강기간(개월)" 
FROM DUAL;
...
SELECT LAST_DAY('1456-02-01') FROM DUAL;
...
/* 형변환 함수 */
-- 문자열(CHAR), 숫자(NUMBER), 날짜(DATE) 끼리 형변환이 가능하다.

/* 문자열로 변환 */
-- TO-CHAR( 날짜, [포멧] ) : 날짜형 데이터를 문자형 데이터로 변경
-- TO-CHAR( 숫자, [포멧] ) : 숫자형 데이터를 문자형 데이터로 변경

-- <숫자 변환 시포맷 패턴>
-- 9 : 숫자 한 칸을 의미, 여러개 작성 시 오른쪽 정렬을 합니다
-- 0 : 숫자 한 칸을 의미, 여러개 작성 시 오른쪽 정렬 + 빈칸이 있으면 숫자 0 추가해줌
-- L : 현재 DB에 설정된 나라의 화폐 기호

SELECT TO_CHAR(1234) FROM DUAL; -- 1,234 -> '1234' 한거임 (숫자 -> 문자 변환한거임)
...
SELECT TO_CHAR(1000000, '9,999,999') || '원' FROM DUAL; -- '1,100,000원'
...
SELECT SYSDATE FROM DUAL; -- 지금 날짜 조회 (년월일시분초.000)

-- 2024/02/29 12:20:54 목요일
SELECT TO_CHAR(SYSDATE, 'RRRR / MM / DD HH24 : MI : SS DAY') FROM DUAL;
...
SELECT TO_CHAR(SYSDATE, 'YYYY"년" HH"월" DD"일" (DY)') FROM DUAL;
...
-- TO_DATE(문자형 데이터, [포맷]) : 문자형 데이터를 날짜로 변경하는것
-- TO_DATE(숫자형 데이터, [포맷]) : 숫자형 데이터를 날짜로 변경하는것
--> 지정된 FORMAT 으로 날짜를 인식함.

SELECT TO_DATE('2024-02-29') FROM DUAL; --DATE TYPE으로 변환
...
-- Y 패턴 : 현재 세기 (21 세기 == 20XX년대 == 2000 년대)
-- R 패턴 : 1 세기를 기준으로 절반(50년 ) 이상인 경우 이전 세기를 나타냄 (1990년대)
...
-- NVL(컬럼명, 컬럼값이 NULL 일 때, 바꿀 값) : NULL 인 컬럼값을 다른값으로 변경하는 함수 
...
-- 해당 컬럼이 NULL 이라면 바꿀값2로 변경


-- EMPLOYEE TABLE에서 BONUS를 받는 직원은 'O' 안받으면  'X'

SELECT EMP_NAME, NVL2(BONUS, 'O', 'X') "보너스 수령"
FROM EMPLOYEE;

/* 선택함수 */
-- 여러가지 경우에 따라서 알맞은 결과를 선택할 수 있다.

-- DECODE(계산식 | 컬럼명, 조건값1, 선택값1, 조건값2, 선택값 2..... , 아무것도 일치하지 않을 때)
...
-- 하나이상의 행을 그룹으로 묶어 연산하여 총합, 평균 등의 하나의 결과행으로 반환하는 함수
...
--MIN(컬럼명) : 최소값
--MAX(컬럼명) : 최대값
--> 타입 제한 없음 ( 숫자 : 대 /소, 날짜 : 과거/ 미래, 문자열 : 문자의 순서)
...
-- COUNT([DISTINCT] 컬럼명) : 중복을 제거한 행의 개수를 헤아려서 리턴
-- COUNT(*) : NULL을 포함한 전체 행 개수를 리턴
-- COUNT(컬럼명) : NULL 제외한 실제 값이 기록된 행 개수를 리턴함
...
--> 왜 한 행이 적을까 ? NULL 값이 포함되지 않아서
-- COUNT(컬럼명)에 의해 NULL을 제외한 실제 값이 있는 행의 개수만 조회


-- EMPLOYEE 테이블에서 성별이 남성인 사원의 수를 조회
SELECT COUNT(*)
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) = '1'; -- 15명

SELECT SUM( DECODE(SUBSTR(EMP_NO, 8, 1), '1', 1, 0 ) ) -- 15명
FROM EMPLOYEE;
